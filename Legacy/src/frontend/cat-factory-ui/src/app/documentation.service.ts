...
export class DocumentationService {
  ...
  public getImportedDatabases(): Observable<ListResponse<ImportedDatabase>> {
    return this.httpClient.get<ListResponse<ImportedDatabase>>([this.baseUrl, 'ImportedDatabases'].join('/'));
  }

  public importDatabase(model: ImportDatabaseRequest): Observable<ImportDatabaseResponse> {
    return this.httpClient.post<ImportDatabaseResponse>([this.baseUrl, 'ImportDatabase'].join('/'), model);
  }

  public getDatabaseDetail(model: DbRequest): Observable<SingleResponse<DatabaseDetail>> {
    return this.httpClient.post<SingleResponse<DatabaseDetail>>([this.baseUrl, 'DatabaseDetail'].join('/'), model);
  }

  public getTable(model: DbRequest): Observable<SingleResponse<TableDetail>> {
    return this.httpClient.post<SingleResponse<TableDetail>>([this.baseUrl, 'Table'].join('/'), model);
  }

  public getView(model: DbRequest): Observable<SingleResponse<ViewDetail>> {
    return this.httpClient.post<SingleResponse<ViewDetail>>([this.baseUrl, 'View'].join('/'), model);
  }
